...
function make_crumbs($post) {
  ...
  while($p->post_parent != 0) {
    $p = new TimberPost($p->post_parent);
    array_unshift($crumbs, array('name' => $p->post_name, 'title' => $p->post_title, 'url' => get_permalink( $p )));
  }
  return $crumbs;
}
...
$context['photo_credits'] = Timber::get_widgets('photo_credits');
